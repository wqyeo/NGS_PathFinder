  ...
  if (dataJSON.items) {
    dataJSON.items.landmark.map(function (x) {
      if (x.disabled === true) {
        ...
        itemsLandmarks.appendChild(createButton("landmark", x.item, "places"));
      }
    });
  }
  ...
  if (dataJSON.items) {
    dataJSON.items.mineral.map(function (x) {
      if (x.disabled === true) {
        ...
        itemsMinerals.appendChild(createButton("mineral", x.item, x.rarity));
      }
    });
  }
  ...
  if (dataJSON.items) {
    dataJSON.items.food.map(function (x) {
      ...
      accordionIcon.innerHTML =
        toggleAccordionFood === x.group_locale_id ? "-" : "+";
      accordionButton.appendChild(accordionIcon);

      accordionButton.innerHTML =
        "{t('sections:regions." + x.group_locale_id + "')}";

      var list = document.createElement("list");
      list.className =
        toggleAccordionFood === x.group_locale_id ? "active" : "";
  ...
  if (dataJSON.items) {
    dataJSON.items.other.map(function (x) {
      if (x.disabled === true) {
        ...
        itemsOther.appendChild(createButton("other", x.item, x.rarity));
      }
    });
  }
